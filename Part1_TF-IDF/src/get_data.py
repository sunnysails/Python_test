...
from src import GrobalParament
...
path1 = base_path + 'data/title_and_abs1/'  # 处理后的标题和摘要
...
def get_text():
    ...
    for files in filelist:
        ...
        soup = BeautifulSoup(open(path + filename + '.xml',encoding='utf-8'), 'html.parser')  # 解析网页
        ...
        if b is None or b.string is None:
            ...
            s = b.string
            abstracts.extend(s)
            f = open(path1 + filename + ".txt", "w+", encoding='utf-8')  # 写入txt文件
            for i in abstracts:
                f.write(i)
            f.close()
            abstracts = []
        ...
        for link in links:
            s1 = link.get_text()
            # print s1
        f = open(newpath + filename + ".txt", "w+", encoding='utf-8')  # 将得到的未处理的文字放在pro_keyword文件夹中
        for i in s1:
            f.write(i)
        f.close()
...
def get_keyword():
    ...
    for files in filelist:
        ...
        f1 = open(newpath + filename + ".txt", "r", encoding='utf-8')
        f2 = open(newpath2 + filename + '.txt', "w+", encoding='utf-8')

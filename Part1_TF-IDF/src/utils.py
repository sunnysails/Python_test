...
def StrToUni(str, *type_list):
    if not type_list:
        if True:  # StrToUni_try(str, 'utf-8'):
            return str.decode('utf-8')
        else:
            print("输入的源文件的编码格式不是utf-8")
    else:
        for type_2 in type_list:
            if True:  # StrToUni_try(str, type_2):
...
def prepro_file(fl_in_url, re_out_url, *wd_be_del):
    in_url = fl_in_url.replace('\\', '/')  # 兼容win和其他系统路径
    out_url = re_out_url.replace('\\', '/')
    fl_in = os.listdir(in_url)
    # out_file=out_url+'/'+GrobalParament.PreprocessResultName
    fl_in.sort(key=lambda x: int(x[:-4]))  # 目录下文件排序
    re_out = open(out_url, 'w', encoding='utf-8')
    i = 0
    for file in fl_in:
        i += 1
        print(i)
        ...
        if os.path.isfile(afile_url):
            afile = open(afile_url, "r", encoding="utf-8")
